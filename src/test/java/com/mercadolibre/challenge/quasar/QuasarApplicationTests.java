...
class QuasarApplicationTests {
	...
	void topSecret_Integration_Ok_Test() throws IOException {
		...
		ResponseEntity<Object> responseEntity = topSecretController.topSecret(topSecretRequest);
		assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
		var body = (TopSecretResponse)responseEntity.getBody();
		assertThat(body.getMessage()).isEqualTo("este es un mensaje secreto");
		assertThat(body.getPosition().getX()).isEqualTo(-487.2859125);
		assertThat(body.getPosition().getY()).isEqualTo(1557.014225);
	}
